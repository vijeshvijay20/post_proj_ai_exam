...
from detect_using_yolov8 import object_detect_v8

windows_object_process = None
windows_noise_process = None
windows_head_process = None
windows_track_process = None
windows_head_object_process = None 
windows_yolov8 = None
   


def start_object_detect_v8():
    global windows_yolov8
    windows_yolov8 = Process(target=object_detect_v8)
    windows_yolov8.start()

def start_windows_tracking_process():
    global windows_track_process
    windows_track_process = Process(target=track_window_changes)
    windows_track_process.start()
...
def start_proctoring_processes():
    ...
    start_windows_tracking_process()
def stop_processes():
    global windows_object_process,windows_yolov8, windows_head_object_process, windows_noise_process, windows_head_process, windows_track_process
    ...
    if windows_head_process:
        ...
        windows_head_process = None
    if windows_track_process:
        windows_track_process.terminate()
        windows_track_process.join()
        windows_track_process = None
