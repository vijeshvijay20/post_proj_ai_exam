...
from start_stop import start_object_detect_v8,start_object_tracking_process,start_proctoring_processes,start_windows_tracking_process
...
def ai_proctoring(request):
    ...
    return HttpResponse("AI procoring bstarted.")


def yolov8_object_tracking(request):
    start_object_detect_v8()
    return HttpResponse("Object tracking with YoloV8 started.")
...
def detect_windows(request):
    ...
    return render(request, 'online_proctoring/window_trac.html')

def head_and_eye(request):
    start_head_and_eye_tracking_process()
    return HttpResponse("Head and Eye tracking started.")
...
def yolov8_tracking(request):
    ...
    return render(request, 'online_proctoring/Yolov8_Tracking.html', {'data': data})
...
def windows_tracking(request):
    ...
    # Pass the data to the template
    return render(request, 'online_proctoring/windows_tracking.html', {'data': data})
